...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_STEERING_GEAR1, DP_TYPE_ENUM},
  {DPID_STEERING_GEAR2, DP_TYPE_ENUM},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,Humidity*10); //VALUE型数据上报;
    mcu_dp_value_update(DPID_BATTERY_PERCENTAGE,BatElect*10); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_STEERING_GEAR1,steering1); //枚举型数据上报;
    mcu_dp_enum_update(DPID_STEERING_GEAR2,steering2); //枚举型数据上报;
}
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_steering_gear1_handle
功能描述 : 针对DPID_STEERING_GEAR1的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_steering_gear1_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char steering_gear1;
    
    steering_gear1 = mcu_get_dp_download_enum(value,length);
    switch(steering_gear1) {
        case 0:			
			steering1 = 0;
			break;       
        case 1:			
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_STEERING_GEAR1, steering_gear1);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_steering_gear2_handle
功能描述 : 针对DPID_STEERING_GEAR2的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_steering_gear2_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char steering_gear2;
    
    steering_gear2 = mcu_get_dp_download_enum(value,length);
    switch(steering_gear2) {
        case 0:			
			steering2 = 0;
			break;       
        case 1:			
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_STEERING_GEAR2, steering_gear2);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    unsigned char ret;
    switch(dpid) {
        case DPID_STEERING_GEAR1:
            //舵机1处理函数
            ret = dp_download_steering_gear1_handle(value,length);
        break;
        case DPID_STEERING_GEAR2:
            //舵机2处理函数
            ret = dp_download_steering_gear2_handle(value,length);
        break;
...
void mcu_open_weather(void)
{
    ...
    /*当获取的参数有和时间有关的参数时(如:日出日落)，需要搭配t.unix或者t.local使用，
		需要获取的参数数据是按照格林时间还是本地时间    */
//	buffer[0] = sprintf(buffer+1,"t.local"); //本地时间 	或使用 buffer[0] = sprintf(buffer+1,"t.unix"); //格林时间    
//    send_len = set_wifi_uart_buffer(send_len, (unsigned char *)buffer, buffer[0]+1);
...
//    unsigned char err_num = 0;
    ...
    if(res == 1) {
        //打开天气返回成功
		printf("weather open is successful\n");
    }else if(res == 0) {
        //打开天气返回失败
        //获取错误码
//        err_num = err; 
...
void weather_data_user_handle(char *name, unsigned char type, const unsigned char *data, char day)
{
    ...
    if(type == 0) { //参数是INT型
        ...
        my_strcpy(value_string, (char *)data);
    }
